...
<Dropdown class="w-full max-w-sm whitespace-normal">
  <div slot="trigger" class="inline-flex items-center text-sm font-medium text-center text-gray-500 hover:text-gray-900 focus:outline-none dark:hover:text-white dark:text-gray-400" on:click={() => {
    fetchNotifications()
  ...
  <ul slot="content" class="rounded divide-y divide-gray-100 shadow dark:bg-gray-800 dark:divide-gray-700">
    ...
    <DropdownItem class="flex space-x-4">
      <Avatar src="/images/friendster.png" rounded />
      <div class="pl-3 w-full">
          <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">{notification.message}</div>
          <div class="text-xs text-blue-600 dark:text-blue-500">{howLongAgo(new Date(notification.createdAt))}</div>
      </div>
    </DropdownItem>
    {/each}
